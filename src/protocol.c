...
const uint8_t FB_CMD_BATTERY[8] = {0x01, 0x08, 0x01, 0x00,
                                   0x02, 0x00, 0x03, 0x00};
const size_t FB_CMD_BATTERY_LEN = 8;

const uint8_t FB_CMD_VERSION[] = {
    0x01, 0x07, 0x01, 0x00, 0x02, 0x00, 0x03, 0x00, 0x04,
    0x00, 0x05, 0x00, 0x06, 0x00, 0x07, 0x00, 0x08, 0x00,
...
const uint8_t FB_CMD_GET_LANGUAGE[8] = {0x0C, 0x02, 0x01, 0x00,
                                        0x02, 0x00, 0x03, 0x00};
const uint8_t FB_CMD_GET_GREET[4] = {0x2B, 0x0F, 0x01, 0x00};
const uint8_t FB_CMD_GET_WEAR[4] = {0x2B, 0x11, 0x01, 0x00};
const uint8_t FB_CMD_GET_LONG_PRESS[8] = {0x2B, 0x17, 0x01, 0x00,
                                          0x02, 0x00, 0x03, 0x00};
const uint8_t FB_CMD_GET_SHORT_PRESS[8] = {0x2B, 0x21, 0x01, 0x00,
                                           0x02, 0x00, 0x03, 0x00};
const uint8_t FB_CMD_GET_SLIDE[8] = {0x2B, 0x1F, 0x01, 0x00,
                                     0x02, 0x00, 0x03, 0x00};
...
const uint8_t FB_CMD_GET_NOISE_CONTROL[6] = {0x2B, 0x19, 0x01,
                                             0x00, 0x02, 0x00};
...
const uint8_t FB_CMD_GET_DOUBLE_CLICK_MBB[6] = {0x01, 0x20, 0x01,
                                                0x00, 0x02, 0x00};
const uint8_t FB_CMD_GET_DOUBLE_CLICK_CALL[4] = {0x01, 0x20, 0x04, 0x00};
const uint8_t FB_CMD_GET_LONG_PRESS_MBB[6] = {0x2B, 0x17, 0x01,
                                              0x00, 0x02, 0x00};
const uint8_t FB_CMD_GET_LONG_PRESS_CALL[6] = {0x2B, 0x17, 0x04,
                                               0x00, 0x05, 0x00};
...
const uint8_t FB_CMD_GET_WEARING_STATUS[6] = {0x2B, 0x25, 0x01,
                                              0x00, 0x02, 0x00};
...
static uint8_t *build_side_pair(uint8_t svc, uint8_t cmd, int left, int right,
                                size_t *len) {
  fb_buf b;
  fb_buf_init(&b);
  fb_buf_put(&b, svc);
...
static uint8_t *build_bool_set(uint8_t svc, uint8_t cmd, int enable,
                               size_t *len) {
  fb_buf b;
  fb_buf_init(&b);
  fb_buf_put(&b, svc);
...
uint8_t *fb_cmd_single_device_setting(int sub, const uint8_t *mac,
                                      size_t *len) {
  fb_buf b;
  fb_buf_init(&b);
  fb_buf_put(&b, 0x2B);
...
const char *const FB_EQ_MODES[4] = {"Default", "Bass boost", "Treble boost",
                                    "Voices"};

const fb_ival FB_NC_MODE_ALIASES[] = {
    {"off", 0},         {"0", 0},    {"cancel", 0},       {"on", 1},
    {"noise", 1},       {"1", 1},    {"aware", 2},        {"awareness", 2},
    {"transparent", 2}, {"pass", 2}, {"pass_through", 2}, {"2", 2},
    {NULL, 0},
};

const fb_ival FB_NC_LEVEL_ALIASES[] = {
    {"general", 0},     {"balance", 0}, {"0", 0},     {"cozy", 1},
    {"comfortable", 1}, {"light", 1},   {"1", 1},     {"ultra", 2},
    {"deep", 2},        {"deeply", 2},  {"depth", 2}, {"2", 2},
    {"dynamic", 3},     {"ai", 3},      {"smart", 3}, {"3", 3},
    {NULL, 0},
};

const fb_ival FB_EQ_MODE_ALIASES[] = {
    {"default", 1},   {"balanced", 1}, {"1", 1},      {"bass", 2},
    {"bassboost", 2}, {"2", 2},        {"treble", 3}, {"trebleboost", 3},
    {"3", 3},         {"voices", 9},   {"voice", 9},  {"clearvoice", 9},
    {"9", 9},         {NULL, 0},
};
...
const fb_feature FB_FEATURABLE[14] = {
    {"greet", "Voice prompts", "Greet (voice prompt)", FB_CMD_GET_GREET,
     sizeof(FB_CMD_GET_GREET), NULL, 0, 0x2B, 0x0F, 0x0E},
    {"wear", "Wear detection", "Wear detection", FB_CMD_GET_WEAR,
     sizeof(FB_CMD_GET_WEAR), NULL, 0, 0x2B, 0x11, 0x10},
    {"ivol", "Intellect volume", "Intellect volume",
     FB_CMD_GET_INTELLECT_VOLUME, sizeof(FB_CMD_GET_INTELLECT_VOLUME), NULL, 0,
     0x2B, 0x23, 0x22},
    {"save", "Saving mode", "Saving mode", FB_CMD_GET_SAVING_MODE,
     sizeof(FB_CMD_GET_SAVING_MODE), NULL, 0, 0x2B, 0x1D, 0x1C},
    {"glat", "Game low latency", "Game low latency",
     FB_CMD_GET_GAME_LOW_LATENCY, sizeof(FB_CMD_GET_GAME_LOW_LATENCY), NULL, 0,
     0x2B, 0x6C, 0x6C},
    {"dual", "Dual connect", "Dual connect", FB_CMD_GET_DUAL_CONNECT,
     sizeof(FB_CMD_GET_DUAL_CONNECT), NULL, 0, 0x2B, 0x2F, 0x2E},
    {"trans", "Translate mode", "Translate mode", FB_CMD_GET_TRANSLATE_MODE,
     sizeof(FB_CMD_GET_TRANSLATE_MODE), NULL, 0, 0x2B, 0x4D, 0x4C},
    {"venh", "Voice enhance", "Voice enhance", FB_CMD_GET_VOICE_ENHANCE,
     sizeof(FB_CMD_GET_VOICE_ENHANCE), NULL, 0, 0x2B, 0x42, 0x41},
    {"32k", "32K HD voice", "32K HD voice", FB_CMD_GET_32K_HD,
     sizeof(FB_CMD_GET_32K_HD), NULL, 0, 0x2B, 0x46, 0x45},
    {"hd", "HD sound switch", "HD sound switch", FB_CMD_GET_HD_SWITCH,
     sizeof(FB_CMD_GET_HD_SWITCH), NULL, 0, 0x2B, 0x5E, 0x5D},
    {"lre", "L/R ear recognition", "L/R ear recognition",
     FB_CMD_GET_LEFT_RIGHT_EAR, sizeof(FB_CMD_GET_LEFT_RIGHT_EAR), NULL, 0,
     0x2B, 0x9A, 0x99},
    {"wind", "Wind mode", "Wind mode", FB_CMD_GET_WIND_MODE,
     sizeof(FB_CMD_GET_WIND_MODE), NULL, 0, 0x2B, 0x95, 0x94},
    {"silent", "Silent upgrade", "Silent upgrade", FB_CMD_GET_SILENT_UPGRADE,
     sizeof(FB_CMD_GET_SILENT_UPGRADE), NULL, 0, 0x2B, 0x70, 0x6F},
    {"health", "Health alerts", "Health alerts", FB_CMD_GET_HEALTH_ALERTS,
     sizeof(FB_CMD_GET_HEALTH_ALERTS), NULL, 0, 0x2B, 0x61, 0x60},
};
