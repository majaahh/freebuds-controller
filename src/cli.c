...
static const char *const LP_NAMES[18] = {
    ...
    "Play/Previous", "NC off/ambient", "Noise control", "Reject call",
    NULL, NULL, "Song recognition", "Freestyle listen", NULL, "Health query",
};

static const char *const DC_NAMES[10] = {
    "Voice assistant", "Play/Pause", "Next track", "NC on/off", "Play/Next",
    "NC on/off/ambient", "NC on/ambient", "Previous track", "Play/Previous",
    "NC off/ambient",
};
...
void fb_scan_freebuds(fb_scan_results *out, int scan_time)
{
    ...
    snprintf(cmd, sizeof(cmd), "bluetoothctl --timeout %d scan on 2>/dev/null", scan_time);
    p = popen(cmd, "r");
    if (p)
    {
    ...
    while (fgets(cmd, sizeof(cmd), p))
    {
        ...
        if (sscanf(cmd, "Device %63s %127[^\n]", addr, name) == 2)
        {
            ...
            {
                out->items = realloc(out->items, (out->count + 1) * sizeof(fb_scan_entry));
...
void print_run_help(void)
{
    ...
    printf("  gesture <type> [<side> <act>]  - Get/set gesture (lp|pinch|slide|dc|triple|lhold|nc)\n");
    printf("  anc [<mode>|level <name|0-3>]  - Get/set noise control (off|on|aware; level: general|cozy|ultra|dynamic)\n");
    printf("  sfx [<name>]                   - Get/set sound effect (default|bass|treble|voices)\n");
    printf("  lang [<code>]                  - Get/set language\n");
    printf("  misc [<name> [on|off]]         - Get/set misc features\n");
    printf("  devices [<action> [<mac|idx>]] - List/connect/disconnect/unpair devices stored on the buds\n");
...
static void print_gesture_help(void)
{
    ...
    printf("  gesture nc <gesture> <side>    - Assign noise control to a gesture\n");
    printf("\n");
    printf("  Side: 1=left, 2=right\n");
    printf("  Gestures: lp (long press), pinch/sp, slide/sl, dc (double click),\n");
    printf("            triple/tri, lhold (light hold)\n");
    printf("\n");
    printf("  Common actions: 0=Voice assistant 1=Play/Pause 2=Next 3=NC on/off 7=Previous 255=None\n");
    printf("  Long press extras: 10=Noise control 11=Reject call 14=Song recognition 15=Freestyle listen 17=Health query\n");
    printf("  Double click extras: 4=Play/Next 5=NC on/off/ambient 6=NC on/ambient 8=Play/Previous 9=NC off/ambient\n");
    printf("  Pinch actions: 0=Translate 1=Voice memo 2=Pairing 3=Freestyle 4=Pinch chat 5=Song recognition\n");
    printf("  Slide actions: 0=Volume 1=Prev/Next track\n");
    printf("  Triple click: 0=Assistant 1=Play/Pause 2=Next 3=NC on/off 4=Noise control\n");
}

static const char *resolve_address(const char *address, char *out, size_t outsz)
{
    char loaded[32];
...
int fb_cmd_scan(int time_sec, const char *save_path)
{
    ...
    if (save_path)
    {
        ...
        if (f)
        {
            ...
            for (i = 0; i < scan.count; i++)
            {
                fprintf(f, "  {\"name\": \"%s\", \"address\": \"%s\", \"rssi\": %d}%s\n",
...
static int run_misc_get(fb_ctrl *ctrl, const char *fname)
{
    ...
    if (feat)
    {
        got = fb_ctrl_get_bool(ctrl, &br, feat->get, feat->get_len, feat->svc, feat->cmd, feat->set_cmd);
        if (got)
        {
            if (br.has_enabled)
    ...
    if (strcmp(fname, "wearst") == 0)
    {
        ...
        if (got)
        {
            ...
            else
            {
                ...
                snprintf(left, sizeof(left), "%s", wr.has_left_ear ? (wr.left_in ? "In" : "Out") : "?");
                snprintf(right, sizeof(right), "%s", wr.has_right_ear ? (wr.right_in ? "In" : "Out") : "?");
    ...
    {
        got = fb_ctrl_get_uint(ctrl, &ur, FB_CMD_GET_DORMANT_TIME, sizeof(FB_CMD_GET_DORMANT_TIME), 0x2B, 0x48);
        if (got && ur.has_value)
    ...
    {
        got = fb_ctrl_get_uint(ctrl, &ur, FB_CMD_QUERY_PAIR, sizeof(FB_CMD_QUERY_PAIR), 0x2B, 0x8F);
        if (got && ur.has_value)
            printf("Pair state: %u\n", ur.value);
        else if (got && ur.has_status)
    ...
    {
        got = fb_ctrl_get_uint(ctrl, &ur, FB_CMD_GET_FIT_CHECK, sizeof(FB_CMD_GET_FIT_CHECK), 0x2B, 0x26);
        if (got && ur.has_value)
    ...
    {
        got = fb_ctrl_get_uint(ctrl, &ur, FB_CMD_GET_FIT_DETECT_VERSION, sizeof(FB_CMD_GET_FIT_DETECT_VERSION), 0x2B, 0x37);
        if (got && ur.has_value)
    ...
    {
        got = fb_ctrl_get_uint(ctrl, &ur, FB_CMD_HEARTBEAT, sizeof(FB_CMD_HEARTBEAT), 0x2B, 0x4E);
        if (got && ur.has_value)
            printf("Heartbeat: %u\n", ur.value);
        else if (got && ur.has_status)
    ...
    {
        got = fb_ctrl_get_bool(ctrl, &br, FB_CMD_GET_BT_MAIN_STATUS, sizeof(FB_CMD_GET_BT_MAIN_STATUS), 0x2B, 0x6A, 0x6A);
    ...
    {
        got = fb_ctrl_get_bool(ctrl, &br, FB_CMD_GET_CLOSE_COVER_REMIND, sizeof(FB_CMD_GET_CLOSE_COVER_REMIND), 0x2B, 0x7F, 0x7F);
        if (got && br.has_enabled)
    ...
    {
        got = fb_ctrl_get_uint(ctrl, &ur, FB_CMD_GET_MUSIC_MODE, sizeof(FB_CMD_GET_MUSIC_MODE), 0x2B, 0x53);
        if (got && ur.has_value)
            printf("Music mode: %u\n", ur.value);
        else if (got && ur.has_status)
    ...
    {
        got = fb_ctrl_get_bool(ctrl, &br, FB_CMD_GET_AMBIENT_SOUND, sizeof(FB_CMD_GET_AMBIENT_SOUND), 0x2B, 0x2C, 0x2C);
        if (got && br.has_enabled)
    ...
    {
        got = fb_ctrl_get_bool(ctrl, &br, FB_CMD_GET_CONNECT_ABILITY, sizeof(FB_CMD_GET_CONNECT_ABILITY), 0x2B, 0x2D, 0x2D);
        if (got && br.has_enabled)
...
static int run_misc(fb_ctrl *ctrl, int argc, char **argv)
{
    ...
    if (argc >= 2 && (is_on_value(argv[1]) || is_off_value(argv[1])))
    {
        ...
        got = fb_ctrl_set_bool(ctrl, &sr, payload, payload_len, feat->svc, feat->set_cmd);
        free(payload);
        if (got && sr.has_success && sr.success)
...
static int run_gesture_get(fb_ctrl *ctrl, int g)
{
    ...
    {
        if (tr.has_left)
            printf("%s: Left=%s", FB_GESTURES[g].name, action_name(g, tr.left, buf, sizeof(buf)));
        else
...
static int run_nc(fb_ctrl *ctrl, int argc, char **argv)
{
    ...
    if (argc == 0)
    {
        ...
        if (!assigned)
            printf("No gesture set to noise control. Use: gesture nc <gesture> <side>\n");
        return 0;
    }
    ...
    {
        fb_setres sr;
        if (!fb_ctrl_set_touch(ctrl, &sr, &FB_GESTURES[g], side, FB_GESTURES[g].nc_action))
        {
            printf("No response\n");
            return 0;
        }
...
static int run_gesture(fb_ctrl *ctrl, int argc, char **argv)
{
    ...
    if (g < 0)
    {
        printf("Unknown gesture '%s'. Use: lp, pinch, slide, dc, triple, lhold, nc\n", argv[0]);
        return 0;
    }
...
static int run_get_noise_mode(fb_ctrl *ctrl)
{
    ...
    printf("No response. Device may not support direct ANC mode control; try 'gesture nc' to assign noise control to a gesture.\n");
    return 0;
}
...
static int run_sfx(fb_ctrl *ctrl, int argc, char **argv)
{
    ...
    if (argc >= 1)
    {
        ...
        {
            printf("Unknown sound effect '%s'. Use: default|bass|treble|voices (or 1|2|3|9)\n", argv[0]);
            return 0;
        }
...
static int run_devices(fb_ctrl *ctrl, int argc, char **argv)
{
    ...
    if (!fb_ctrl_single_device_setting(ctrl, &sr,
                                       ...
                                       strcmp(action, "disconnect") == 0 ? 2 : 3,
                                       macbuf))
    {
        printf("no response\n");
        return 0;
    ...
    if (sr.has_result_code)
    {
        ...
        if (code == 0 || code == (int)FB_STATUS_OK)
            printf("%s %s\n", strcmp(action, "connect") == 0 ? "Connected" :
                              strcmp(action, "disconnect") == 0 ? "Disconnected" : "Unpaired",
                   macbuf);
        else
            printf("%s failed: result=%d\n",
                   strcmp(action, "connect") == 0 ? "Connect" :
                   strcmp(action, "disconnect") == 0 ? "Disconnect" : "Unpair",
                   code);
    }
...
static int run_info(fb_ctrl *ctrl)
{
    ...
    if (fb_ctrl_get_battery(ctrl, &bat))
        printf("battery: L:%d%%  R:%d%%  Box:%d%%\n", bat.left_battery, bat.right_battery, bat.box_battery);
    else
    ...
    {
        fb_boolres br;
        if (fb_ctrl_get_bool(ctrl, &br, FB_FEATURABLE[i].get, FB_FEATURABLE[i].get_len,
                             FB_FEATURABLE[i].svc, FB_FEATURABLE[i].cmd, FB_FEATURABLE[i].set_cmd))
        {
            if (br.has_enabled)
...
static int is_known_command(const char *s)
{
    static const char *const cmds[] = {
        "help", "bat", "ver", "info", "gesture", "anc", "nc", "sfx", "lang",
        "raw", "misc", "devices", "dev", "lp", "sp", "slide", "dc", "triple",
        "pinch", "lhold",
    };
    size_t i;
...
{
    return strcmp(s, "lp") == 0 || strcmp(s, "sp") == 0 || strcmp(s, "slide") == 0 ||
           strcmp(s, "dc") == 0 || strcmp(s, "triple") == 0 || strcmp(s, "pinch") == 0 ||
           strcmp(s, "lhold") == 0;
}
