...
uint8_t *fb_build_frame(const uint8_t *payload, size_t payload_len,
                        size_t *out_len) {
...
fb_frames *fb_parse_frames(const uint8_t *data, size_t len) {
  ...
  while (offset < len) {
    ...
    frames->items =
        realloc(frames->items, (frames->count + 1) * sizeof(fb_frame));
    ...
    {
      uint16_t crc_recv = (uint16_t)(((uint16_t)remaining[total - 2] << 8) |
                                     remaining[total - 1]);
