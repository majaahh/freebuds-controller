...
fb_frames *fb_ctrl_send(fb_ctrl *c, const uint8_t *cmd, size_t cmd_len, double timeout)
{
    fb_frames *frames;
    uint8_t *frame;
    size_t frame_len;
...
static const fb_frame *match_frame_or_first(const fb_frames *frames, int svc, int cmd)
{
...
static int extract_byte_at(const uint8_t *data, size_t len, size_t offset, uint8_t *out)
{
    if (offset < len)
    {
        *out = data[offset];
...
static const uint8_t *tlv_value(const fb_tlvs *tlvs, uint8_t tag, size_t *vlen)
{
    size_t i;
...
static void set_raw_hex(char *out, size_t outsz, const uint8_t *data, size_t len)
{
...
static void decode_version_string(const uint8_t *v, size_t vlen, char *out, size_t outsz)
{
    size_t i, n = 0;
...
int fb_ctrl_get_version(fb_ctrl *c, fb_version *out)
{
    ...
    for (i = 0; i < tlvs.count; i++)
    {
        ...
        default:
            ...
            {
                snprintf(out->other[out->other_count], sizeof(out->other[0]), "%s", buf);
...
int fb_ctrl_get_bool(fb_ctrl *c, fb_boolres *out, const uint8_t *get_cmd, size_t get_len, int svc, int get_cmd_id, int set_cmd_id)
{
    fb_frames *frames;
    const fb_frame *f;
    fb_tlvs tlvs;
    ...
    for (i = 0; i < tlvs.count; i++)
    {
        if (tlvs.items[i].len == 1 && (tlvs.items[i].value[0] == 0 || tlvs.items[i].value[0] == 1))
        {
            out->has_enabled = 1;
...
int fb_ctrl_set_bool(fb_ctrl *c, fb_setres *out, const uint8_t *set_cmd, size_t set_len, int svc, int set_cmd_id)
{
    fb_frames *frames;
    const fb_frame *f;
...
int fb_ctrl_get_uint(fb_ctrl *c, fb_uintres *out, const uint8_t *cmd, size_t len, int svc, int cmd_id)
{
    fb_frames *frames;
    const fb_frame *f;
    fb_tlvs tlvs;
...
static uint8_t *gesture_set_payload(const fb_gesture *g, int side, int action, size_t *len)
{
...
int fb_ctrl_set_touch(fb_ctrl *c, fb_setres *out, const fb_gesture *g, int side, int action)
{
    fb_frames *frames;
    const fb_frame *f;
    fb_tlvs tlvs;
    ...
    for (i = 0; i < tlvs.count; i++)
    {
        ...
        if ((t->tag == 3 || t->tag == 6) && t->len == 1)
        {
            ...
            out->has_success = 1;
            snprintf(out->result_name, sizeof(out->result_name), "Result %d", t->value[0]);
            fb_tlvs_free(&tlvs);
            fb_frames_free(frames);
            return 1;
        }
...
static void anc_set_from_status(fb_setres *out, const uint8_t *data, size_t len)
{
    fb_tlvs tlvs;
    const uint8_t *val;
    size_t vlen;
...
int fb_ctrl_set_anc_state_extended(fb_ctrl *c, fb_setres *out, int mode, int level)
{
    fb_frames *frames;
    const fb_frame *f;
    uint8_t *payload;
...
int fb_ctrl_get_anc_state(fb_ctrl *c, fb_ancres *out)
{
    ...
    frames = fb_ctrl_send(c, FB_CMD_GET_ANC_STATE, sizeof(FB_CMD_GET_ANC_STATE), 2.0);
    ...
    if (tlvs.count == 0 && extract_byte_at(f->data, f->data_len, 2, &b) && in_nc_modes(b))
    {
        out->has_mode = 1;
        out->mode = b;
        fb_tlvs_free(&tlvs);
...
int fb_ctrl_get_anc_mode_level(fb_ctrl *c, fb_ancres *out)
{
    ...
    frames = fb_ctrl_send(c, FB_CMD_GET_ANC_MODE_LEVEL, sizeof(FB_CMD_GET_ANC_MODE_LEVEL), 2.0);
...
int fb_ctrl_query_nr_mode(fb_ctrl *c, fb_ancres *out)
{
    ...
    frames = fb_ctrl_send(c, FB_CMD_QUERY_NOISE_REDUCTION_MODE, sizeof(FB_CMD_QUERY_NOISE_REDUCTION_MODE), 2.0);
...
int fb_ctrl_get_noise_control_lr(fb_ctrl *c, fb_ancres *out)
{
    ...
    frames = fb_ctrl_send(c, FB_CMD_GET_NOISE_CONTROL, sizeof(FB_CMD_GET_NOISE_CONTROL), 2.0);
...
int fb_ctrl_get_language(fb_ctrl *c, fb_langres *out)
{
    ...
    memset(out, 0, sizeof(*out));
    frames = fb_ctrl_send(c, FB_CMD_GET_LANGUAGE, sizeof(FB_CMD_GET_LANGUAGE), 2.0);
    ...
    for (i = 0; i < tlvs.count; i++)
    {
        ...
        {
            snprintf(out->other[out->other_count], sizeof(out->other[0]), "%.127s", buf);
            out->other_tags[out->other_count] = t->tag;
            out->other_count++;
        }
    }
...
int fb_ctrl_get_wearing_status(fb_ctrl *c, fb_wearres *out)
{
    ...
    memset(out, 0, sizeof(*out));
    frames = fb_ctrl_send(c, FB_CMD_GET_WEARING_STATUS, sizeof(FB_CMD_GET_WEARING_STATUS), 2.0);
...
int fb_ctrl_set_dormant_time(fb_ctrl *c, fb_setres *out, int option, int seconds)
{
    fb_frames *frames;
    const fb_frame *f;
    uint8_t *payload;
...
int fb_ctrl_exit_fit_check(fb_ctrl *c, fb_setres *out)
{
    ...
    memset(out, 0, sizeof(*out));
    frames = fb_ctrl_send(c, FB_CMD_EXIT_FIT_CHECK, sizeof(FB_CMD_EXIT_FIT_CHECK), 2.0);
...
static void parse_bonded_device(const uint8_t *data, size_t len, fb_device *dev)
{
...
static void parse_device_report(const uint8_t *data, size_t len, fb_device *dev, char *addr_out, size_t addr_sz, int *type)
{
    fb_tlvs tlvs = fb_parse_tlv(data, len);
    size_t i;
    *type = -1;
...
int fb_ctrl_get_bonded_devices(fb_ctrl *c, fb_devices *out)
{
    ...
    memset(out, 0, sizeof(*out));
    frames = fb_ctrl_send(c, FB_CMD_GET_DEVICES_BONDED, sizeof(FB_CMD_GET_DEVICES_BONDED), 2.0);
    if (!frames)
        return 0;
    ...
    for (i = 0; i < frames->count; i++)
    {
        ...
        if (fr->cmd == 0x31)
        {
            ...
            if (addr[0])
            {
                ...
                if (!dev)
                {
                    out->items = realloc(out->items, (out->count + 1) * sizeof(fb_device));
        ...
        else if (fr->cmd == 0x36)
        {
            ...
            parse_device_report(fr->data, fr->data_len, &tmp, addr, sizeof(addr), &type);
            if (addr[0])
            {
...
int fb_ctrl_single_device_setting(fb_ctrl *c, fb_setres *out, int sub, const char *mac)
{
    fb_frames *frames;
    uint8_t macb[6];
    uint8_t *payload;
        ...
        if (fr->svc == 0x2B && fr->cmd == 0x33)
        {
            ...
            for (j = 0; j < tlvs.count; j++)
            {
                ...
                else if (t->tag == 31 && t->len == 1)
                {
                    ...
                    out->has_success = 1;
                    snprintf(out->result_name, sizeof(out->result_name), "%d", t->value[0]);
                    has = 1;
                }
            }
            fb_tlvs_free(&tlvs);
    ...
    if (!found && is_status_response(frames->items[0].data, frames->items[0].data_len))
    {
        out->has_status = 1;
        out->status = parse_status_code(frames->items[0].data, frames->items[0].data_len);
        out->has_success = 1;
        out->success = (out->status == 0 || out->status == FB_STATUS_OK);
        found = 1;
    }
...
int fb_ctrl_set_primary_device(fb_ctrl *c, fb_setres *out, const char *mac)
{
    ...
    {
        out->has_success = 0;
        set_raw_hex(out->raw_hex, sizeof(out->raw_hex), frames->items[0].data, frames->items[0].data_len);
    }
    fb_frames_free(frames);
    return 1;
}
...
int fb_ctrl_get_cloud_version(fb_ctrl *c, char *out_hex, size_t outsz)
{
    ...
    frames = fb_ctrl_send(c, FB_CMD_CLOUD_VERSION, sizeof(FB_CMD_CLOUD_VERSION), 2.0);
...
int fb_ctrl_get_ota_params(fb_ctrl *c, char *out_hex, size_t outsz)
{
    ...
    frames = fb_ctrl_send(c, FB_CMD_GET_OTA_PARAMS, sizeof(FB_CMD_GET_OTA_PARAMS), 2.0);
